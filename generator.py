...
class Expr:
    ...
    evaluate_mod = NotImplemented # Haven't been able to do this bullshit without changing important stuff
    ...
    def log10_estimate(self, var_values: dict) -> float:
        ...
        if self.kind == "var":
            if self.value is not None:
                ...
                if v is None:
                    raise ValueError(
                        f"No numeric value provided for variable '{self.name}'"
                    )
            if v == 0:
                return -inf
            return log10(abs(v))
...
if __name__ == "__main__":
    ...
    # Placeholder modulo calculation, Yall can change the script for this on github, right? 
    # Im relatively new to this github stuff really.
    # This is gonna be disabled for now.
    if 0 != 0:
        try:
            mod = 10 ** 9 + 7
